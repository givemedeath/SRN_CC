...
- [x] Follow the MADR shape used by `docs/adr/0003-*.md`: `## Context and Problem Statement`,
      `## Decision Drivers`, `## Considered Options`, `## Decision Outcome`, `### Consequences`,
      `### Rejected options`.
- [x] Record that `PLAN.md:170-176` specifies an opt-in downloader with three pinned SHA-256 values
      and `PLAN.md:228` specifies opt-in compatibility tests, and that neither is built.
- [x] Record the reasoning: `PLAN.md:228` itself forbids the oracle from being the authoritative
...
- [x] Record the reopening condition, the way `docs/adr/0003-*.md:182` defines a compliance gate.

**Exit Criteria**
- [x] The ADR names the specific tests that discharge the oracle's intended role.
- [x] No provenance keyword subject to the `AuditVendoredSources.ps1` grep appears in any `.cs`,
...
- [x] `enum LogLevel { Trace, Debug, Info, Warn, Error }`.
- [x] `sealed record LogRecord(DateTimeOffset TimestampUtc, LogLevel Level, string Category,
      string Message, string? EventCode, IReadOnlyDictionary<string,string>? Data,
      string? ExceptionType, string? ExceptionMessage)`.
- [x] `ILogSink : IDisposable` with `void Write(LogRecord record)` and `void Flush()`.
- [x] `IAppLogger.Log(LogLevel, string category, string message, Exception? = null,
      IReadOnlyDictionary<string,string>? = null)`.
- [x] `NullAppLogger.Instance` as a no-op singleton — the default for every trailing optional
      `IAppLogger?` parameter added anywhere in this milestone.
- [x] Declarations only. No sink implementation, no file I/O, no rotation in this slice.

**Exit Criteria**
- [x] A `LogRecord` whose `Message` contains `\n` serializes to exactly one physical line.
- [x] `NullAppLogger.Instance.Log(...)` never throws for any input, including a null exception and
      an empty category.
- [x] `SRN.CC.Core` still has no UI, Formats, or Infrastructure dependency; `ArchitectureTests`
      needs no edit.
...
- [x] `enum StartupCheckSeverity { Ok, Degraded, Blocking }`.
- [x] `sealed record StartupCheckResult(string CheckId, StartupCheckSeverity Severity,
      string Summary, IReadOnlyList<string> Details, DiagnosticCode? Code = null)`.
- [x] `IStartupCheck` with `string CheckId { get; }` and
      `Task<StartupCheckResult> RunAsync(CancellationToken cancellationToken = default)`.
- [x] `sealed record StartupReport(IReadOnlyList<StartupCheckResult> Results)` exposing `HasBlocking`
      and `Worst`.
- [x] `sealed record AppPaths(string Root)` with `CacheDatabasePath` (`Root/cache-v1.sqlite`),
      `SettingsPath` (`Root/settings.json`), `LogDirectory` (`Root/Logs`), and `static Default`
      resolving `%LOCALAPPDATA%\SRN.CC`.
- [x] Declarations only. No preflight runner and no concrete check in this slice.

**Exit Criteria**
- [x] `AppPaths.Default.CacheDatabasePath` equals the literal path `SqliteCacheService` computes
...
- [x] `StartupReport.Worst` returns the maximum severity across results and `Ok` for an empty list.
- [x] `HasBlocking` is true if and only if at least one result is `Blocking`.
...
- [x] `enum SchemaKind { Project, Settings, Cache, Manifest }`,
      `enum SchemaOpenMode { Unsupported, Current, ReadOnlyNewer }`.
- [x] `static class SchemaVersions` with `Project = 1`, `Settings = 1`, `Cache = 1`,
      `Manifest = "1.0"`, plus `Current(SchemaKind)` and `Classify(SchemaKind, int fileVersion)`.
- [x] `IJsonSchemaMigration` with `Kind`, `FromVersion`, `ToVersion`, and
      `JsonObject Apply(JsonObject document)`.
- [x] `SchemaMigrationPipeline(SchemaKind, IEnumerable<IJsonSchemaMigration>)` with
...
- [x] Ship **zero** registered migrations. 1 → 1 is an identity no-op returning `applied = []`.
- [x] Add a **test-only** `FakeV1ToV2Migration` (and a 2 → 3 sibling) inside the test file, never in
      production code.

**Exit Criteria**
- [x] A 1 → 2 → 3 chain applies both migrations in order and reports both in `applied`.
- [x] Requesting version 3 with only 1 → 2 registered returns `false` with an error naming the
      missing step, rather than silently skipping it.
- [x] An identity upgrade does not mutate the input `JsonObject` — asserted by reference and by
      serialized text, because `ProjectStore` preserves unknown fields through
      `ProjectPreferences.RawRootNode`.
- [x] `Classify` truth table verified for versions 0, 1, and 2 across all four `SchemaKind` values.
...
- [x] Add `SRNCCVersionPrefix` (`1.0.0`), `SRNCCProductName`, and `SRNCCCompany` to
      `eng/Versions.props`.
- [x] In `Directory.Build.props`, derive `Version`, `AssemblyVersion` (`$(SRNCCVersionPrefix).0`),
      `FileVersion`, `InformationalVersion`, `Product`, and `Company`, and set
      `<IncludeSourceRevisionInInformationalVersion>false</IncludeSourceRevisionInInformationalVersion>`.
- [x] Replace the `"1.0"` / `"1.0.0"` literals at `ProvenanceManifestGenerator.cs:84-85` with
      `SchemaVersions.Manifest` and a cached read of
      `Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()`.
- [x] This is the milestone's **only** edit to `Directory.Build.props`.

**Exit Criteria**
- [x] Two manifest generations over identical input differ only in `GeneratedUtc`.
- [x] `AppVersion` equals the runtime informational version and contains no `+` character.
- [x] `eng/Versions.props`'s `SRNCCVersionPrefix` equals the runtime `AssemblyInformationalVersion`
      for all five production assemblies.
- [x] No `[A-Za-z]:\` sequence appears anywhere in generated manifest text.
- [x] `dotnet build -c Release` is clean; no existing test changes behaviour.
...
- [x] `README.md`: what the application is, supported platform (`win-x64` only), how to obtain and
      run the release, and a table of contents linking into `docs/operator/`.
- [x] `INSTALL-AND-RUN.md`: extracting the release archive, first launch, what the preflight reports,
      and the fact that inputs are read-only and never modified.
- [x] `FILE-LOCATIONS.md`: the `%LOCALAPPDATA%\SRN.CC` layout — `cache-v1.sqlite`, `settings.json`,
      `Logs\` — plus what is safe to delete and what is not.
- [x] `RECOVERY.md`: what a quarantined cache or settings file means and how to recover; how an
      interrupted publish is rolled back at startup; what a `.publication-journal.json` file is.
- [x] `PROJECT-FILE-FORMAT.md`: the `.srnccproj` schema-1 shape for humans, the newer-schema
      read-only rule, and the unknown-field preservation guarantee.
- [x] `MANIFEST-FORMAT.md`: the `<basename>.srncc-manifest.json` fields and how to verify a published
      HAK against it by hand.
- [x] `GPU-REQUIREMENTS.md`: rendering goes through ANGLE (OpenGL ES via EGL) on Windows; what
      happens when the capability probe falls short; the three-viewport limit.
- [x] `TROUBLESHOOTING.md`: reading `Logs\srncc.log`, common degraded-preflight messages, and what
...
**Exit Criteria**
- [x] Every path, file name, and environment variable named in the documentation exists in the code
      as written — verified by opening the referencing source, not from memory.
- [x] `README.md` links resolve relative to the repository root.
- [x] Nothing in `docs/operator/` describes a feature this milestone defers.

### S7 — Release policy data (no scripts)
Owns MODIFY `eng/publish-policy.json`; NEW `eng/release-policy.json`.
- [x] Append `av_libglesv2.dll`, `libSkiaSharp.dll`, and `libHarfBuzzSharp.dll` to `requiredFiles`.
      **No `specialOrigins` entries for the three natives.** All three are already origin-resolved
      from the deps manifest's `native` asset group, so an entry would be strictly redundant while
      making `AuditPublish.ps1`'s `Add-Origin` (lines 24-31) raise *ambiguous origins* the moment a
      package version bump makes the frozen literal disagree with the deps-derived string. This is
      why the pre-existing `e_sqlite3.dll` native carries no entry either.
- [x] Append `.log`, `.srnccproj`, `.srncc`, `.sqlite-wal`, `.sqlite-shm`, and `.zip` to
      `forbiddenExtensions`.
- [x] Append `nwn_erf`, the third-party toolset name, `SRNCC_CORPUS_ROOT`, `sqlite3_64.dll`, and
...
- [x] Create `eng/release-policy.json` with `zipEntryTimestampUtc`, `zipExcludedFiles`
      (`["publish-inventory.json"]`), `nonSingleFileMarkers`, `minimumFileCount`, and
      `releaseNameTemplate`.
- [x] Pure data only. This slice writes no PowerShell.

**Exit Criteria**
- [x] `tools/VerifyBuild.ps1` still passes end to end with the extended policy — run it locally
...
- [x] `eng/release-policy.json` parses and every key is consumed by a wave-1 script's documented
...
- [x] `internal static class CorpusGate` with `IsRequired` (`SRNCC_REQUIRE_CORPUS`), `IsEnabled`
...
- [x] `RequireCorpusRoot()` precedence, in this order: `IsRequired` and the root is missing or does
...
- [x] Replace the triplicated `[SetUp]` gating at `CorpusIndexAcceptanceTests.cs:22-34`,
...
- [x] `IdleWorkingSetTests.cs`, `[Category("Performance")]` and additionally gated on
      ...
      750 MiB.
- [x] `docs/evidence/corpus-index-baseline.json`, shaped like
...
**Exit Criteria**
- [x] With no corpus environment variables set, every corpus fixture still ignores exactly as it does
      today — the default `Category!=Corpus&Category!=Performance` run is unchanged.
- [x] `SRNCC_REQUIRE_CORPUS=1` with no `SRNCC_CORPUS_ROOT` produces a **failure** naming
      `SRNCC_CORPUS_ROOT`, not an ignore.
- [x] The idle probe stays skipped inside a plain corpus run unless `SRNCC_RUN_IDLE_PROBE=1` is set.
- [x] `docs/evidence/corpus-index-baseline.json` is populated from a real recorded run, with the
      machine environment captured. If the corpus is unavailable to the slice, leave the numeric
      fields null and say so explicitly — do not invent values. **Null path taken:** no corpus was
      available (`SRNCC_CORPUS_ROOT` unset), so the file carries `"recorded": false`, a
      `notRecordedReason`, and a `populate` block naming the exact commands and the test methods that
      produce each missing number. Structural constants (117 / 187,943 / 212 / 4, the 10.0 s and
      750 MiB thresholds) are filled in; every measured value is null. **S23 must not report the
      corpus tier as verified until this file is populated on a machine with the corpus.**
